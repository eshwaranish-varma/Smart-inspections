...
import { assignInspection } from "@/lib/db/inspection-service";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (user.role !== "supervisor") {
      return NextResponse.json({ error: "Only supervisors can assign inspections" }, { status: 403 });
    }
    ...
    const inspection = await assignInspection(id, assignedTo, user.id, comments);
