...
export default function Form483Preview({
  ...
  const cityStateZip = [metadata.city, metadata.state, metadata.zip_code]
    .filter(Boolean)
    .join(", ");
  const establishmentType = (metadata.establishment_type || "")
    .replace(/_/g, " ")
  ...
  const invNames = (metadata.investigators || [])
    .map((inv) => inv.name || "")
    .filter(Boolean)
    .join(", ");
  const invNamesAndTitles = (metadata.investigators || [])
    .map((inv) => `${inv.name || ""}, ${inv.title || ""}`)
    .filter(Boolean);
